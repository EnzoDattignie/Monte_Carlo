...
int main (int argc, char *argv[]) {
    int ndim, n , seed = 22, n_cycles = 100000, current_cycle = 0, *Liste_Verlet;
    double lmax = 0.012, P, kT = 0.05;
    char *input_file = "input_Phase_N.xyz";
    if (argc > 1) {
    ...
    xyz = fopen("res/out_Phase_N.xyz","w");
    save_xyz(xyz, pos,0,n,ndim,kT);

    log = fopen("res/out_Phase_N.log","w");
    ...
    for (i = 0; i < n_iter; i++) {
        ...
        if (i%(100*100) == 100*100-1) {
            printf("Current cycle : %d\n",current_cycle);
            save_xyz(xyz, pos,current_cycle,n,ndim,kT);
        }
        if (i%(1000*100) == 1000*100-1) {
            u_tail(&utail,pos,box,Liste_Verlet,n,ndim,rcut,uc);
            pressure(&P, pos, box, Liste_Verlet, n, ndim, kT, rcut);
            save_log(log, utot+utail, box, kT, P, n, ndim, current_cycle, lmax, seed);
            n ++;
